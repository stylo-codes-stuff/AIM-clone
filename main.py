...
def connect(auth):
    ...
    if memCheck(room):
        ...
        print("memory good")
    #ummm i forgot what these if statements are for exactly...
    #i think this first one is for if the room or user doesn't exist
    if not room or not name:
        return
    if room not in rooms:  
        leave_room(room)
        return
    #If the connection is accepted join the room and send all the corresponding data
    join_room(room)
